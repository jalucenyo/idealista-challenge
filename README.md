...
**Por:** Jose Antonio Luceño Castilla

En este reto me he centrado es las "reglas" de calculo para las puntuaciones de los anuncios, para que el sistema siga los principios SOLID sea extensible y facil de mantener, cada regla tienen su propia clase que implementa  los metodos del interface "QualityRule". A su vez cada regla es un Bean (Componente de Spring), que nos permite con la inversión de dependencias, agregar nuevas reglas sin modificar el código existente. 

Las reglas definidas se iyectan en una lista en el servicio QualityCalculator, que se encarga de recorrer cada anuncio y aplicar todas la reglas definidas sobre el mismo.

Del mismo modo en la parte de testing, nos permite probar el funcionamiento de cada regla de forma individual.

El resto de la aplicación se han creado los endpoints, solicitados y el mapeo a las clases de dto que devuelve el api. 
Para probar la aplicación de una forma mas fácil se ha añadido [Swagger](http://localhost:8080/swagger-ui.htm). 
